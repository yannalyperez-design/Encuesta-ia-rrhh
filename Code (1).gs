...
function guardarRespuesta(payload) {
  ...
  sheet.appendRow([
    new Date(), email, payload.nombre, payload.apellido, payload.area, payload.cedula,
...
function getRespuestasAdmin() {
  ...
  return data.slice(1).filter(function (r) { return r[1] !== ''; }).map(function (r) {
    return {
      timestamp: r[0], email: r[1], nombre: r[2], apellido: r[3], area: r[4], cedula: r[5],
      respuestas: JSON.parse(r[6] || '{}')
    };
  });
}
...
function inicializarHojas() {
  ...
  crearHojaSiNoExiste_(ss, 'Respuestas', ['Timestamp', 'Email', 'Nombre', 'Apellido', 'Area', 'Cedula', 'RespuestasJSON']);
